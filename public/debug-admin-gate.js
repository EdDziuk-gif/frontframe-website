(function installAdminGateDebug() {
  ...
  window.showAdmin = async function showAdmin() {
	...
	console.log(tag + ' 2. session.access_token present: ' + !!(session && session.access_token));
	console.log(tag + ' 2. session.user.email: ' + (session && session.user && session.user.email ? session.user.email : '(none)'));

	// Step 3 - fetch reviewers
	console.log(tag + ' 3. Fetching /admin/reviewers ...');
	var reviewerList = [];
	try {
	  var res = await fetch(WORKER_URL + '/admin/reviewers', {
		headers: {
	  ...
	  if (!res.ok) {
		...
		console.log(tag + ' 3. Reviewer emails: ' + reviewerList.map(function(r){ return r.email; }).join(', '));
		console.log(tag + ' 3. dev_access values: ' + JSON.stringify(reviewerList.map(function(r){ return { email: r.email, dev_access: r.dev_access }; })));
	  }
	} catch (e) {
	...
	var userEmail = session && session.user && session.user.email ? session.user.email : '';
	var me = reviewerList.find(function(r){ return r.email === userEmail; });
	...
	console.log(tag + ' 4. dev_access on match: ' + (me ? me.dev_access : '(no match)'));

	// Step 5 - dev tools link
	var devLink = document.getElementById('dev-tools-link');
	console.log(tag + ' 5. dev-tools-link element found: ' + !!devLink);
	var shouldShow = me && me.dev_access === true;
