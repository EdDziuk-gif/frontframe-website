...
**Exception (added 2026-07-24):** this "never deleted" default assumes the
feature being debugged stays stable. When the underlying implementation is
being actively rearchitected (not just modified), a debug file written
against the old implementation isn't a dormant asset -- it's stale and
would silently misbehave if reactivated. In that case, delete the debug
file along with the old implementation, rather than keeping it "Ready" in
the registry below. (First applied when `debug-otp.js` was removed
alongside the Worker-proxied OTP flow it debugged, once admin.html moved
to direct Supabase SDK OTP calls.)
