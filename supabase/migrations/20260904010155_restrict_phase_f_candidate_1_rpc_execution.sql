...
--
-- This migration's version (20260904010155) is recorded in the live
-- project's remote migration history as
-- "restrict_phase_f_candidate_1_rpc_execution", but the SQL actually
-- executed against production was never committed to this repository.
-- This file is a faithful reconstruction built from:
--
--   - worker/phase_f_schema.sql (already in this repository — a
--     preserved source artifact whose header states it was meant to
--     be applied via the SQL editor, matching this migration's scope
--     almost exactly);
--   - a privileged, read-only catalog inspection of the live database
--     (exact column lists, constraints, indexes for both tables; the
--     exact live function bodies via pg_get_functiondef) run
--     2026-09-14/15;
--   - Decision 0032 (frontframe-obsidian-vault), which authorizes this
--     proposal/promulgation/incident workflow.
--
-- SPLIT NOTE: the remote history records this work as two migrations,
-- 55 seconds apart ("restrict_..." then "harden_..."). worker/
-- phase_f_schema.sql presents both functions already in their final,
-- search_path-hardened form, with no record of an intermediate,
-- unhardened state. The split adopted here — this file creates the
-- tables and functions and restricts EXECUTE to service_role;
-- 20260904010250_harden_... adds the search_path hardening — is a
-- reasoned inference from the two migration names and their sub-
-- minute gap, not a recovered historical fact. The combined end state
-- of both files matches the confirmed live definitions exactly.
-- ============================================================
...
ALTER TABLE public.constitution_amendment_proposals ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.authorization_incidents ENABLE ROW LEVEL SECURITY;
...
-- Supabase grants EXECUTE to PUBLIC by default unless explicitly revoked;
-- keep callers on the authenticated Worker path where authority and
-- lifecycle checks are applied.
